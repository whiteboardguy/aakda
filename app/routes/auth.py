from app.utils.print_utils import printStat
from starlette.status import HTTP_500_INTERNAL_SERVER_ERROR, HTTP_200_OK, HTTP_401_UNAUTHORIZED, HTTP_409_CONFLICT
...
async def create_user(
    ...
    if db.scalars(select(models.User).where(models.User.username == user_input.username)).first():
        raise HTTPException(status_code=HTTP_409_CONFLICT, detail="Username provided already exists.")
    if db.scalars(select(models.User).where(models.User.email == user_input.email)).first():
        raise HTTPException(status_code=HTTP_409_CONFLICT, detail="Email address provided already in use.")
    ...
    if new_user.password is unhashed:
        printStat("c", "Password hashing system has failed.")
        raise HTTPException(status_code=HTTP_500_INTERNAL_SERVER_ERROR)
...
async def login_user(
    ...
    tg_user = db.scalars(select(models.User).where(models.User.username == input.username)).first()
